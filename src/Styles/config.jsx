// export const address = "http://43.201.181.250";
// export const kakaoAPI = `${address}/kakao/callback`;

// export const emailSendAPI = `${address}/emails/auth`;

// export const authNumAPI = `${address}/emails/check`;

// export const signinAPI = `${address}/members/signup`;

// export const loginAPI = `${address}/members/login`;

// export const mainListAPI = `${address}/posts?page=0&size=10&sort=createdAt,DESC`;

// export const detailAPI = `${address}/posts/{id}`;

// export const kakaoIcon =
//   "data:image/svg+xml;base64,";

// export const arrowIcon =
//   "data:image/svg+xml;base64,";

// export const emptyHeart =
//   "https://tumblbug.com/wpa/1d4978f729cd37a38573e85f399b74c1.png";

// export const fullHeart =
//   "https://tumblbug.com/wpa/730f4f57f22fa624ea87b013550472b8.png";

// export const greyCircle =
//   "https://media.vlpt.us/images/playck/post/2bba80cf-a966-47dd-bd70-bacf9c91bc65/CF230510-940F-4F37-86CE-B5A83B109C41.png";

// export const blueCircle =
//   "https://media.vlpt.us/images/playck/post/d09d60c6-210b-4092-a2df-f727d0b30354/7BC376DA-2AEA-4C71-923D-6307C137DD1E.png";
